...
#        logger.info("tree_idx: {}, {}".format(type(tree_idx), tree_idx))
#        logger.info("p: {}, {}".format(type(p), p))
...
class Memory(object):  # stored as ( s, a, r, s_ ) in SumTree
    ...
    """
    def __init__(self, capacity=1000000, epsilon=1e-2, alpha=0.6, beta=0.4,
            beta_increment_per_sampling=1e-3, abs_err_upper=1.0):
    ...
    def sample(self, n):
        b_idx, b_memory, ISWeights = np.empty((n,), dtype=np.int32), np.empty((n,), dtype=np.int32), np.empty((n,))
        pri_seg = self.tree.total_p / n       # priority segment
        self.beta = np.min([1., self.beta + self.beta_increment_per_sampling])  # max = 1

        min_prob = np.min(self.tree.tree[-self.tree.capacity:]) / self.tree.total_p     # for later calculate ISweight
        min_prob += 1e-10   # to avoid division by 0 error
        for i in range(n):
